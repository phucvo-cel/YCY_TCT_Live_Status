...
| LS-B013 | 2026-08-31 | Medium | Before sharing or publishing this HTML file outside the team, review the hardcoded `DEFAULT_CARTO_KEY` in `tileLayer()` (`LS-D048`): confirm whether the shared team key is still appropriate for the new audience and domain, or request a dedicated/rotated CARTO key instead. | The key in `tileLayer()` is confirmed appropriate for the actual sharing destination and domain, or replaced, before the file is shared or published outside the team. | Explicit user decision, 2026-08-31: "tạm thời xài key đó đi, lúc cbi share html thì tôi sẽ đọc lại sau" |
