# Control Tower Engagements with YCH — Index

This is the entry point for the working file and knowledge base. The content
used to live as one file, `CT_Working_File_and_Knowledge_Base_v13.md`
(834 lines), which is now split by part for narrower loading. That original
file is kept unmodified, as the full-text archive, at
[archive/CT_Working_File_and_Knowledge_Base_v13.md](./archive/CT_Working_File_and_Knowledge_Base_v13.md);
nothing below is a copy, it is the same content relocated. Do not read the
...
- [CT_TCT_SPVB_Working_File_v11.md](./CT_TCT_SPVB_Working_File_v11.md) — Track 2's current scope, effective 27 August 2026: Vietnam, shipper Suntory PepsiCo Vietnam Beverage (SPVB), superseding the Indonesian scope above (shipper referred to there as FFI). Forked from a parent file not held in this repository; carries the network, lanes, frozen worked example, milestone-4 activation, the twenty-two exception catalogue (V9), the playbook rules editor (VD25 to VD31), and YCH's own scoping-meeting statement (V10, VD32 to VD35), which overrides the earlier assumed network and dates. Its own decision series now runs VD1 to VD35, inheriting Part C's doctrine and D1 to D88 by reference.
...
number (`L<n>`) for every subsection, so a targeted lookup can jump straight
to a line range instead of reading the whole file.

## Version and change log

Project reference, version 13, last updated 25 August 2026, evening. The file
covers two distinct engagement tracks with YCH (contact: Sardar Ahmed):
...
Future change-log entries: add them here, at the top of this section, exactly
as before. Note which split file (A, B, or C) each change touched.
