...
- `KB/CT_TCT_SPVB_Working_File_v11.md`: Track 2's current scope from 27 August 2026, Vietnam, shipper Suntory PepsiCo Vietnam Beverage (SPVB), superseding Part C's original Indonesian scope (shipper FFI). Network, lanes, the frozen worked example, the milestone-4 activation, the twenty-two exception catalogue, and the playbook rules editor are here. YCH's own scoping-meeting statement (V10) overrides the earlier assumed network and dates: three named warehouses (DOP, OPMT / Sóng Thần, Bắc Ninh), one HCM warehouse first, implementation September 2026 to January 2027. It inherits doctrine, architecture and most decisions from Part C by reference.
