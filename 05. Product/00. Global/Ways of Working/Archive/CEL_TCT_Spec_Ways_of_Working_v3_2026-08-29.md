...
CEL, YCH Transport Control Tower, SPVB Vietnam. Version 3, 29 August 2026. Owner: Pierre.
...
│   │   ├── 1.1 Live Status/                       pod
│   │   └── 1.2 Operational Performance Dashboard/ pod
│   ├── CT2. Exception Handling/
│   │   ├── 2.1 User-based Exception List/         pod
│   │   └── 2.2 Recommendation and Resolution Engine/ pod
│   ├── CT3. Continuous Improvement/
│   │   ├── 3.1 Reports and Advanced Analytics/    pod
│   │   └── 3.2 Recommended Initiatives and Actions/ pod
│   │
...
└── Released/                              approved copies only, read only
...
|---|---|---|
| CT1 End to End Visibility | 1.1 Live status, 1.2 Operational performance dashboard | Pods |
| CT2 Exception Handling | 2.1 User-based exception list, 2.2 Recommendation and resolution engine | Pods |
| CT3 Continuous Improvement | 3.1 Reports and advanced analytics, 3.2 Recommended initiatives and actions | Pods |
...
The six sub-modules are the ones on the transport control tower stack slide, and they are the level a pod works at. A specification, a screen inventory and a pod mockup belong to a sub-module. A register belongs to the CT tool above it, because a register crosses sub-modules.
---
...
*Happy path.*
Given a trip with a planned ETA at the site, when neither truck queue in nor truck checked in has been recorded more than PB-P19 (4 hours) after that ETA, then the tower raises PB-16, gives it to the warehouse supervisor at the site, and shows it in the work queue ranked by its deadline.

*Unhappy paths.*
- **Truck is in the yard.** Given the truck has queued in but not checked in, when PB-P19 passes, then PB-16 is not raised, because the truck is on site; the yard wait is watched by PB-P15 instead.
...
*Open questions.* Is PB-P19 four hours or two? To settle with YCH at workshop W3, by 15 September.
...
## 5. Working, Released, and how something gets approved

```
   Working                 Ready for approval           Released              Shared
┌──────────────┐         ┌────────────────────┐     ┌──────────────┐     ┌─────────────┐
│ analyst      │  ──▶    │ version named in   │ ──▶ │ v1.2_2026-   │ ──▶ │ devs        │
│ edits freely │         │ the file, this is  │     │ 09-11        │     │ Thibaud     │
│ no ceremony  │         │ the revert point   │     │ read only    │     │ client, PDF │
└──────────────┘         └────────────────────┘     └──────────────┘     └─────────────┘
        ▲                          │
        └──────────────────────────┘
...
1. You finish a change and name the version inside the file, or in Google Docs version history for the few files that are still there. That marker is what we roll back to.
2. Ask Pierre. He approves, or reverts. That is the whole disagreement mechanism: no argument, one click.
3. On approval, the file is copied into Released with its version and date, and the index row is updated.
...
## 6. The mockup: prototype freely, integrate once
...
```
  1. Master mockup        2. Sub-module build     3. Sub-module     4. Sub-module
     release  [Pierre]       [Pod]                   review            integration
                                                     [Pierre]          [Pierre]
┌──────────────────┐     ┌───────────────────┐   ┌──────────────┐  ┌──────────────────┐
│ improving the    │copy │ a new version of  │   │ approves, or │  │ the approved     │
│ mockup, shared   │ ──▶ │ its own sub-      │──▶│ asks for     │─▶│ change is ported │
│ parts and sub-   │     │ module, inside    │   │ another pass │  │ into the master, │
│ module work      │     │ the real shell    │   │              │  │ never merged     │
│ 06. Mockup/      │     │ CT2.1_exception_  │   │              │  │ file to file     │
│ Released         │     │ list_from_v0.16   │   │              │  │                  │
└──────────────────┘     └───────────────────┘   └──────────────┘  └────────┬─────────┘
         ▲                         ▲                                        │
         └─────────────────────────┼────────────────────────────────────────┘
                                   │   a new master is released, one line added
                                   │   to the changelog
                                   └── the pod refreshes its copy before
...
- **Decisions** go in the working file, with what was traded away.
- **Screens** are the master mockup, pointed at from each CT folder by the screen inventory.
...
2. Name every threshold, never write a number inline. `PB-P19`, not "4 hours".
